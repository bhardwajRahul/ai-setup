// Behaviour lock for the vendored caliber-jev-compaction library (MIT). This is
...
describe('decisions', () => {
  ...
  it('honours truncateHeadChars, including a zero head', () => {
    ...
    expect(kept[2]?.toolResults?.[0]?.text).toBe(
      `${original.slice(0, 50)}\n[caliber-jev-compaction truncated ${total - 50} chars of this tool result; re-run the tool if needed]`,
    );
    ...
    expect(noHead[2]?.toolResults?.[0]?.text).toBe(
      `[caliber-jev-compaction truncated ${total} chars of this tool result; re-run the tool if needed]`,
    );
  });
});
